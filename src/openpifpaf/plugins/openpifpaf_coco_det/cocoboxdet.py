...
from .cocoboxkp import CocoBoxKp
...
class CocoBoxDet(openpifpaf.datasets.DataModule):
    # cli configurable
    ...
    def __init__(self):
        super().__init__()
        cifdet = openpifpaf.headmeta.CifDet('cifdet', 'cocoboxdet', COCO_CATEGORIES)
        cifdet.upsample_stride = self.upsample_stride
        self.head_metas = [cifdet]

    @classmethod
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('data module CocoBoxDet')

        group.add_argument('--cocoboxdet-train-annotations',
                           default=cls.train_annotations)
        group.add_argument('--cocoboxdet-val-annotations',
                           default=cls.val_annotations)
        group.add_argument('--cocoboxdet-train-image-dir',
                           default=cls.train_image_dir)
        group.add_argument('--cocoboxdet-val-image-dir',
                           default=cls.val_image_dir)

        group.add_argument('--cocoboxdet-square-edge',
                           default=cls.square_edge, type=int,
                           help='square edge of input images')
        assert not cls.extended_scale
        group.add_argument('--cocoboxdet-extended-scale',
                           default=False, action='store_true',
                           help='augment with an extended scale range')
        group.add_argument('--cocoboxdet-orientation-invariant',
                           default=cls.orientation_invariant, type=float,
                           help='augment with random orientations')
        group.add_argument('--cocoboxdet-blur',
                           default=cls.blur, type=float,
                           help='augment with blur')
        assert cls.augmentation
        group.add_argument('--cocoboxdet-no-augmentation',
                           dest='cocoboxdet_augmentation',
                           default=True, action='store_false',
                           help='do not apply data augmentation')
        group.add_argument('--cocoboxdet-rescale-images',
                           default=cls.rescale_images, type=float,
                           help='overall rescale factor for images')

        group.add_argument('--cocoboxdet-upsample',
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # cocodet specific
        cls.train_annotations = args.cocoboxdet_train_annotations
        cls.val_annotations = args.cocoboxdet_val_annotations
        cls.train_image_dir = args.cocoboxdet_train_image_dir
        cls.val_image_dir = args.cocoboxdet_val_image_dir

        cls.square_edge = args.cocoboxdet_square_edge
        cls.extended_scale = args.cocoboxdet_extended_scale
        cls.orientation_invariant = args.cocoboxdet_orientation_invariant
        cls.blur = args.cocoboxdet_blur
        cls.augmentation = args.cocoboxdet_augmentation
        cls.rescale_images = args.cocoboxdet_rescale_images
        cls.upsample_stride = args.cocoboxdet_upsample
